...
识别对象是红、绿、蓝方块各 2 个，共 6 个目标，摆在 3×2 网格纸的格心上；
USB 相机架在网格侧上方，机械臂回零姿态（大臂竖直、小臂水平向前）时不遮挡网格。

| 现场识别对象与工作环境 | USB 相机（贴在 iPad 背面当支架） |
|---|---|
| ![识别对象](images/real_objects.jpg) | ![相机](images/real_camera_mount.jpg) |
...
现场屏幕上的落格结果（`GRID OK`，6 个格位逐行给出类别与 `5/5 帧` 票数）：

![现场识别画面](images/real_rqt_grid.jpg)
